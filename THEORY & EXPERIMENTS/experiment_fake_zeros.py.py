...
# 1. ЗАГРУЗКА ДАННЫХ
# ============================================================
# Генерируем случайные экспоненциальные интервалы (Пуассоновский процесс)
fake_intervals = np.random.exponential(scale=1.0, size=2_000_000)
# Кумулятивная сумма даёт "фейковые нули"
fake_zeros = np.cumsum(fake_intervals)

# И дальше работаем с fake_zeros вместо zeros
zeros = fake_zeros
...
# ВАЖНО: для синтетических нулей используем ОТДЕЛЬНЫЙ файл кэша!
cache_file = 'gram_indices_2M_FAKE.npy'
...
if os.path.exists(cache_file):
    ...
    print("\nВычисление индексов Грама для 2 000 000 синтетических нулей...")
